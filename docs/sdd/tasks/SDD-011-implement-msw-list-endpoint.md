...
- Node-smoke (теперь vitest integration test `src/shared/api/mocks/handlers/auctions-list.test.ts`, часть `pnpm test:run`) покрывает 7 сценариев / 33 assertion: default body, pagination `per_page`+`page`, пустая страница за диапазоном, фильтр `auc_type`, текстовый `cargo_num`, malformed JSON → 422 с проверкой `content-type: application/problem+json`, flip `is_oldest`. Browser-smoke через Playwright против dev-сервера (`e2e/msw-browser.spec.ts`, часть `pnpm test:e2e`) подтверждает, что воркер регистрируется и fetch из page-context возвращает mock-данные.
