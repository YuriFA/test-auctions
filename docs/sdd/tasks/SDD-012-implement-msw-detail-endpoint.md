...
- Node-smoke `scripts/msw-detail-smoke.mjs` покрывает 6 сценариев / 25 assertion: полная форма `AuctionShowResponse` на `downLeading` (включая проверку, что `main.order_uid` отличается от path-UUID по D-011), restriction `hide_bets_history` на `finishedConfirmed`, связка `no_view_cargo_price` + `hide_points_address_and_contacts` на `fixPriceHidden`, `hide_points_address_and_contacts` на `downHiddenContacts`, 404 + `application/problem+json` + поля `code`/`title`/`message` на неизвестном UUID, отсутствие over-match на `/auctions/{uuid}/bets`.
