...
- `SDD-001..014` завершены. `SDD-015` (URL-контракт фильтров), `SDD-016` (request builder), `SDD-017` (list query + composition), `SDD-018` (фильтры UI), `SDD-019` (UI redesign), `SDD-020` (карточка списка) завершены. `SDD-021+` — detail/bets/bet-форма/мутации.
...
- **SDD-021: restriction-флаги читаются прямо из VM, без central-handler'а.** SDD-022 не начат — detail тянет `hidePointsAddressAndContacts`/`noViewCargoPrice`/`hideBetsHistory`/`canSetBet` как 4 boolean'а из VM. Когда SDD-022 land'нет, замена — одна точка в mapper'е (заменить прямые чтения на `deriveAuctionRestrictions(trading)`), UI не трогается. Задокументировано явно, чтобы будущий рефактор не "размазал" правила обратно по компонентам (предупреждение SDD-022).
