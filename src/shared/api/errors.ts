...
function isValidationProblem(body: unknown): body is ValidationProblem {
  return (
    isProblemDetail(body) && 'errors' in body && Array.isArray((body as { errors: unknown }).errors)
  )
}

export function normalizeApiError(response: Response | undefined, body: unknown): ApiError {
