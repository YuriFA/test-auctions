...
assert("browser fetch returns 2 items (per_page=2)", result.json?.data?.length === 2, `got ${result.json?.data?.length}`);
assert("browser fetch meta.total === 10", result.json?.meta?.total === 10, `got ${result.json?.meta?.total}`);
assert("browser fetch meta.per_page === 2", result.json?.meta?.per_page === 2);
assert("browser fetch has auction_uuid", typeof result.json?.data?.[0]?.main?.auction_uuid === "string");
