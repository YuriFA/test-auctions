...
async function postList(body) {
  const init = body === undefined ? { method: "POST" } : {
    method: "POST",
...

{
  ...
  assert("default data length === 10", Array.isArray(json?.data) && json.data.length === 10, `got ${json?.data?.length}`);
  ...
  assert("returned UUIDs match seed set", JSON.stringify(returnedUuids) === JSON.stringify([...seedUuids].sort()));
}
...

{
  ...
  assert("paginate meta.last_page === 4", json?.meta?.last_page === 4, `got ${json?.meta?.last_page}`);
...

{
  ...
  const includesLeading = (json?.data ?? []).some((item) => item?.main?.auction_uuid === seedAuctionUuids.downLeading);
...

{
  ...
  assert("cargo_num=MSK-001 returns downLeading", json?.data?.[0]?.main?.auction_uuid === seedAuctionUuids.downLeading);
}
...

{
  ...
  assert("sort flips between is_oldest true/false", newestFirst !== oldestFirst, `newest=${newestFirst} oldest=${oldestFirst}`);
}
